...
# UI
# -----------------------------
st.title("📊 Quant Research Terminal (Full Research Suite)")
...
for t in selected:
    if t not in st.session_state.weights:
        st.session_state.weights[t] = 1.0

for t in list(st.session_state.weights.keys()):
    if t not in selected:
        del st.session_state.weights[t]
...
if mode == "Manual":
    ...
    for t in selected:
        default_weight = float(st.session_state.weights.get(t, 1.0))

        val = st.sidebar.slider(
            t, 0.0, 1.0, default_weight, key=f"w_{t}"
        )

        st.session_state.weights[t] = val
        manual.append(val)

    manual = np.array(manual)
    weights = manual / manual.sum() if manual.sum() > 0 else np.ones(len(manual)) / len(manual)

st.session_state.weights = {t: weights[i] for i, t in enumerate(selected)}

# -----------------------------
# PORTFOLIO
# -----------------------------
weights_array = np.array([st.session_state.weights[t] for t in selected])
port_ret = returns[selected] @ weights_array
...
st.write("Weights:", weights_array)
